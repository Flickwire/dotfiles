...
if [[ "$ID" != "amzn" || "$VERSION_ID" != "2" ]]; then
    printf 'Error: this user-data script supports Amazon Linux 2 only.\n' >&2
    exit 1
fi

yum install -y curl git gnupg2 groff-base htop less sudo tar tmux unzip util-linux vim-enhanced zsh
